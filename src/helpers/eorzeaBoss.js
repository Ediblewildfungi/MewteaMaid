const eorzeaBoss = [
    {
        "id": 2,
        "name": "Dungeons (Endgame)",
        "frozen": true,
        "encounters": [
            {
                "id": 4500,
                "name": "Neverreap",
                "cnName": "",
                "nickname": ["0"],

            },
            {
                "id": 4501,
                "name": "The Fractal Continuum",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4502,
                "name": "Saint Mocianne's Arboretum",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4503,
                "name": "Pharos Sirius (Hard)",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4504,
                "name": "The Antitower",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4505,
                "name": "The Lost City of Amdapor (Hard)",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4506,
                "name": "Sohr Kai",
            },
            {
                "id": 4507,
                "name": "Hullbreaker Isle (Hard)",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4508,
                "name": "Xelphatol",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4509,
                "name": "The Great Gubal Library (Hard)",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 4,
        "name": "Trials (Extreme)",
        "frozen": true,
        "encounters": [
            {
                "id": 1027,
                "name": "Bismarck",
                "cnName": "Bismarck",
                "nickname": ["Bismarck"],
            },
            {
                "id": 1028,
                "name": "Ravana",
                "cnName": "Ravana",
                "nickname": ["Ravana"],
            },
            {
                "id": 1029,
                "name": "Thordan",
                "cnName": "Thordan",
                "nickname": ["Thordan"],
            },
            {
                "id": 1031,
                "name": "Sephirot",
                "cnName": "Sephirot",
                "nickname": ["Sephirot"],
            },
            {
                "id": 1033,
                "name": "Nidhogg",
                "cnName": "Nidhogg",
                "nickname": ["Nidhogg"],
            },
            {
                "id": 1034,
                "name": "Sophia",
                "cnName": "Sophia",
                "nickname": ["Sophia"],
            },
            {
                "id": 1035,
                "name": "Zurvan",
                "cnName": "Zurvan",
                "nickname": ["Zurvan"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 5,
        "name": "Void Ark",
        "frozen": true,
        "encounters": [
            {
                "id": 2000,
                "name": "Cetus",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2001,
                "name": "Irminsul",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2002,
                "name": "Cuchulainn",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2003,
                "name": "Echidna",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 6,
        "name": "Alexander: Gordias (Story)",
        "frozen": true,
        "encounters": [
            {
                "id": 14,
                "name": "Oppressor",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 15,
                "name": "The Cuff of the Father",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 16,
                "name": "Living Liquid",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 17,
                "name": "The Manipulator",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5000,
                "name": "Faust",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 7,
        "name": "Alexander: Gordias (Savage)",
        "frozen": true,
        "encounters": [
            {
                "id": 18,
                "name": "Oppressor",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 19,
                "name": "The Cuff of the Father",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 20,
                "name": "Living Liquid",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 21,
                "name": "The Manipulator",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5001,
                "name": "Faust",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 8,
        "name": "The Weeping City of Mhach",
        "frozen": true,
        "encounters": [
            {
                "id": 2004,
                "name": "Arachne Eve",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2005,
                "name": "Forgall",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2006,
                "name": "Ozma",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2007,
                "name": "Calofisteri",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 9,
        "name": "Alexander: Midas (Story)",
        "frozen": true,
        "encounters": [
            {
                "id": 22,
                "name": "Ratfinx Twinkledinks",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 23,
                "name": "Vortexer",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 24,
                "name": "Quickthinx Allthoughts",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 25,
                "name": "Brute Justice",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5002,
                "name": "Hummelfaust",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5003,
                "name": "Blaster",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5004,
                "name": "Brawler",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5005,
                "name": "Swindler",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 10,
        "name": "Alexander: Midas (Savage)",
        "frozen": true,
        "encounters": [
            {
                "id": 26,
                "name": "Ratfinx Twinkledinks",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 27,
                "name": "The Cuff of the Son",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 28,
                "name": "Quickthinx Allthoughts",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 29,
                "name": "Brute Justice",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5006,
                "name": "Hummelfaust",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        }
    },
    {
        "id": 12,
        "name": "Alexander: The Creator (Story)",
        "frozen": true,
        "encounters": [
            {
                "id": 30,
                "name": "Refurbisher 0",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 31,
                "name": "Lamebrix Strikebocks",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 32,
                "name": "Cruise Chaser",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 33,
                "name": "Alexander Prime",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5007,
                "name": "Faust Z",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps (3.55b+)",
                "compact": "Standard (3.55b+)"
            },
            {
                "name": "Non-Standard Comps (3.55b+)",
                "compact": "Non-Standard (3.55b+)"
            },
            {
                "name": "Standard Comps (Echo)",
                "compact": "Standard (Echo)"
            },
            {
                "name": "Non-Standard Comps (Echo)",
                "compact": "Non-Standard (Echo)"
            }
        ]
    },
    {
        "id": 13,
        "name": "Alexander: The Creator (Savage)",
        "frozen": true,
        "encounters": [
            {
                "id": 34,
                "name": "Refurbisher 0",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 35,
                "name": "Lamebrix Strikebocks",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 36,
                "name": "Cruise Chaser",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 37,
                "name": "Alexander Prime",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 5008,
                "name": "Faust Z",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 3,
            "max": 3.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps (3.55b+)",
                "compact": "Standard (3.55b+)"
            },
            {
                "name": "Non-Standard Comps (3.55b+)",
                "compact": "Non-Standard (3.55b+)"
            },
            {
                "name": "Standard Comps (Echo)",
                "compact": "Standard (Echo)"
            },
            {
                "name": "Non-Standard Comps (Echo)",
                "compact": "Non-Standard (Echo)"
            }
        ]
    },
    {
        "id": 14,
        "name": "Dungeons (Endgame)",
        "frozen": true,
        "encounters": [
            {
                "id": 4510,
                "name": "Ala Mhigo",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4511,
                "name": "Kugane Castle",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4512,
                "name": "The Temple of the Fist",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4513,
                "name": "The Drowned City of Skalla",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4514,
                "name": "Hells' Lid",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4515,
                "name": "The Fractal Continuum (Hard)",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4516,
                "name": "The Swallow's Compass",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4517,
                "name": "The Burn",
            },
            {
                "id": 4518,
                "name": "Saint Mocianne's Arboretum (Hard)",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4519,
                "name": "The Ghimlyt Dark",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 1
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 2
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 3
            }
        ]
    },
    {
        "id": 15,
        "name": "Trials (Extreme)",
        "frozen": true,
        "encounters": [
            {
                "id": 1036,
                "name": "Susano",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 1037,
                "name": "Lakshmi",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 1038,
                "name": "Shinryu",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 1040,
                "name": "Byakko",
                "cnName": "极白虎",
                "nickname": ["极白虎", "白虎"],
                "bossCnServer": 3
            },
            {
                "id": 1041,
                "name": "Tsukuyomi",
                "cnName": "极月读",
                "nickname": ["极月读", "月读"],
                "bossCnServer": 3
            },
            {
                "id": 1043,
                "name": "Suzaku",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 1044,
                "name": "Seiryu",
                "cnName": "极青龙",
                "nickname": ["极青龙", "青龙"],
                "bossCnServer": 3
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 16,
        "name": "Omega: Deltascape (Story)",
        "frozen": true,
        "encounters": [
            {
                "id": 38,
                "name": "Alte Roite",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 39,
                "name": "Catastrophe",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 40,
                "name": "Halicarnassus",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 41,
                "name": "Exdeath",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            }
        ]
    },
    {
        "id": 17,
        "name": "Omega: Deltascape (Savage)",
        "frozen": true,
        "encounters": [
            {
                "id": 42,
                "name": "Alte Roite",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 43,
                "name": "Catastrophe",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 44,
                "name": "Halicarnassus",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 45,
                "name": "Exdeath",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 46,
                "name": "Neo Exdeath",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            }
        ]
    },
    {
        "id": 18,
        "name": "The Royal City of Rabanastre",
        "frozen": true,
        "encounters": [
            {
                "id": 2008,
                "name": "Mateus, the Corrupt",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2009,
                "name": "Hashmal, Bringer of Order",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2010,
                "name": "Rofocale",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2011,
                "name": "Argath Thadalfus",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 1
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 2
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 3
            }
        ]
    },
    {
        "id": 19,
        "name": "The Unending Coil of Bahamut",
        "frozen": true,
        "difficulty":100,
        "encounters": [
            {
                "id": 1039,
                "name": "Bahamut Prime",
                "cnName": "绝巴哈姆特-4.0",
                "nickname": ["绝巴哈姆特-4.0"],
                "bossCnServer": 3
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 20,
        "name": "Omega: Sigmascape (Story)",
        "frozen": true,
        "encounters": [
            {
                "id": 47,
                "name": "Phantom Train",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 48,
                "name": "Demon Chadarnook",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 49,
                "name": "Guardian",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 50,
                "name": "Kefka",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            }
        ]
    },
    {
        "id": 21,
        "name": "Omega: Sigmascape (Savage)",
        "frozen": true,
        "encounters": [
            {
                "id": 51,
                "name": "Phantom Train",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 52,
                "name": "Demon Chadarnook",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 53,
                "name": "Guardian",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 54,
                "name": "Kefka",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 55,
                "name": "God Kefka",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            }
        ]
    },
    {
        "id": 22,
        "name": "The Ridorana Lighthouse",
        "frozen": true,
        "encounters": [
            {
                "id": 2012,
                "name": "Famfrit, the Darkening Cloud",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2013,
                "name": "Belias, the Gigas",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2014,
                "name": "Construct 7",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2015,
                "name": "Yiazmat",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 1
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 2
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 3
            }
        ]
    },
    {
        "id": 23,
        "name": "The Weapon's Refrain",
        "frozen": true,
        "difficulty": 100,
        "encounters": [
            {
                "id": 1042,
                "name": "The Ultima Weapon",
                "cnName": "绝究极神兵-4.0",
                "nickname": ["究极神兵-4.0", "绝神兵-4.0", "绝究极神兵-4.0", "神兵-4.0"],
                "amount": "amount"
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 24,
        "name": "Omega: Alphascape (Story)",
        "frozen": true,
        "difficulty":100,
        "encounters": [
            {
                "id": 56,
                "name": "Chaos",
                "cnName": "",
                "nickname": ["卡奥斯","o9s"],
            },
            {
                "id": 57,
                "name": "Midgardsormr",
                "cnName": "",
                "nickname": ["o10s"],
            },
            {
                "id": 58,
                "name": "Omega",
                "cnName": "",
                "nickname": ["o11s"],
            },
            {
                "id": 59,
                "name": "Omega-M and Omega-F",
                "cnName": "",
                "nickname": ["o12s"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 25,
        "name": "Omega: Alphascape (Savage)",
        "frozen": true,
        "encounters": [
            {
                "id": 60,
                "name": "Chaos",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 61,
                "name": "Midgardsormr",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 62,
                "name": "Omega",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 63,
                "name": "Omega-M and Omega-F",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 64,
                "name": "The Final Omega",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            },
            {
                "name": "Standard Comps (Echo)",
                "compact": "Standard (Echo)",
                "area": 1
            },
            {
                "name": "Non-Standard Comps (Echo)",
                "compact": "Non-Standard (Echo)",
                "area": 1
            },
            {
                "name": "Standard Comps (Echo)",
                "compact": "Standard (Echo)",
                "area": 2
            },
            {
                "name": "Non-Standard Comps (Echo)",
                "compact": "Non-Standard (Echo)",
                "area": 2
            },
            {
                "name": "Standard Comps (Echo)",
                "compact": "Standard (Echo)",
                "area": 3
            },
            {
                "name": "Non-Standard Comps (Echo)",
                "compact": "Non-Standard (Echo)",
                "area": 3
            }
        ]
    },
    {
        "id": 26,
        "name": "The Orbonne Monastery",
        "frozen": true,
        "encounters": [
            {
                "id": 2016,
                "name": "Mustadio",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2017,
                "name": "Agrias",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2018,
                "name": "The Thunder God",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 2019,
                "name": "Ultima, The High Seraph",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 4,
            "max": 4.5,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 1
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 2
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 3
            }
        ]
    },
    {
        "id": 27,
        "name": "Dungeons (Endgame)",
        "frozen": false,
        "encounters": [
            {
                "id": 4520,
                "name": "Amaurot",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4521,
                "name": "The Twinning",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4522,
                "name": "Akadaemia Anyder",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4523,
                "name": "The Grand Cosmos",
                "cnName": "",
                "nickname": ["0"],
            },
            {
                "id": 4524,
                "name": "Anamnesis Anyder",
                "cnName": "",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 1
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 2
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 3
            }
        ]
    },
    {
        "id": 28,
        "name": "Trials (Extreme)",
        "frozen": false,
        "difficulty":100,
        
        "encounters": [
            {
                "id": 1045,
                "name": "Titania",
                "cnName": "缇坦妮雅",
                "nickname": ["缇坦妮雅", "极妖精", "妖精王"]
            },
            {
                "id": 1046,
                "name": "Innocence",
                "cnName": "无瑕灵君",
                "nickname": ["无瑕灵君", "肥宅王", "极肥宅", "极肥宅王"]
            },
            {
                "id": 1049,
                "name": "Hades",
                "cnName": "哈迪斯",
                "nickname": ["哈迪斯", "极哈迪斯", "极哈"]
            },
            {
                "id": 1051,
                "name": "The Ruby Weapon I",
                "cnName": "红玉神兵门神",
                "nickname": ["RubyWeapon","红玉神兵1","红玉神兵"]
            },
            {
                "id": 1052,
                "name": "The Ruby Weapon II",
                "cnName": "红玉神兵本体",
                "nickname": ["RubyWeapon2","红玉神兵2","红玉神兵本体"]
            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps (5.0)",
                "compact": "Standard (5.0)",
                "area": 1
            },
            {
                "name": "Non-Standard Comps (5.0)",
                "compact": "Non-Standard (5.0)",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 29,
        "name": "Eden's Gate",
        "frozen": false,
        "difficulty":101,
        "encounters": [
            {
                "id": 65,
                "name": "Eden Prime",
                "cnName": "至尊伊甸",
                "nickname": ["至尊伊甸", "1S", "1s", "e1s", "E1S"]



            },
            {
                "id": 66,
                "name": "Voidwalker",
                "cnName": "虚无行者",
                "nickname": ["虚无行者", "2S", "2s", "e2s", "E2S"]
            },
            {
                "id": 67,
                "name": "Leviathan",
                "cnName": "利维亚桑",
                "nickname": ["利维亚桑", "3S", "3s", "e3s", "E3S"]
            },
            {
                "id": 68,
                "name": "Titan",
                "cnName": "泰坦",
                "nickname": ["泰坦", "4S", "4s", "e4s", "E4S"]
            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps (5.0)",
                "compact": "Standard (5.0)",
                "area": 1
            },
            {
                "name": "Non-Standard Comps (5.0)",
                "compact": "Non-Standard (5.0)",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 30,
        "name": "Ultimates (Stormblood)",
        "frozen": false,
        "difficulty":100,
        "encounters": [
            {
                "id": 1047,
                "name": "The Unending Coil of Bahamut",
                "cnName": "绝巴哈姆特",
                "nickname": ["绝巴哈", "巴哈","绝巴哈姆特","巴哈姆特"],
                "bossCnServer": 3

            },
            {
                "id": 1048,
                "name": "The Weapon's Refrain",
                "cnName": "绝究极神兵",
                "nickname": ["究极神兵", "究极","绝究极神兵","绝神兵","神兵"],
                "bossCnServer": 3
            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 31,
        "name": "The Copied Factory",
        "frozen": false,
        "encounters": [
            {
                "id": 2020,
                "name": "Serial-Jointed Command Model",
                "cnName": "尼尔1",
                "nickname": ["0"],
            },
            {
                "id": 2021,
                "name": "Hobbes",
                "cnName": "尼尔2",
                "nickname": ["0"],
            },
            {
                "id": 2022,
                "name": "Engels",
                "cnName": "尼尔3",
                "nickname": ["0"],
            },
            {
                "id": 2023,
                "name": "9S-Operated Walking Fortress",
                "cnName": "尼尔4",
                "nickname": ["0"],
            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 1
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 2
            },
            {
                "name": "default",
                "compact": "default",
                "default": true,
                "area": 3
            }
        ]
    },
    {
        "id": 32,
        "name": "Ultimates",
        "frozen": false,
        "difficulty":100,
        "encounters": [
            {
                "id": 1050,
                "name": "The Epic of Alexander",
                "cnName": "绝亚历山大",
                "nickname": ["绝亚", "亚历山大"],
                "bossCnServer": 3

            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    },
    {
        "id": 33,
        "name": "Eden's Verse",
        "frozen": false,
        "encounters": [
            {
                "id": 69,
                "name": "Ramuh",
                "cnName": "拉姆",
                "nickname": ["拉姆", "5S", "5s", "e5s", "E5S"],
                "bossCnServer": 3
            },
            {
                "id": 70,
                "name": "Ifrit and Garuda",
                "cnName": "伊芙利特与迦楼罗",
                "nickname": ["伊芙利特与迦楼罗", "6S", "6s", "e6s", "E6S"],
                "bossCnServer": 3
            },
            {
                "id": 71,
                "name": "The Idol of Darkness",
                "cnName": "暗之巫女",
                "nickname": ["暗之巫女", "7S", "7s", "e7s", "E7S"],
                "bossCnServer": 3
            },
            {
                "id": 72,
                "name": "Shiva",
                "cnName": "希瓦",
                "nickname": ["希瓦", "8S", "8s", "e8s", "E8S"],
                "bossCnServer": 3
            }
        ],
        "brackets": {
            "min": 5,
            "max": 5.2,
            "bucket": 0.1,
            "type": "Patch"
        },
        "partitions": [
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "area": 1,
                "default": true
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 1
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 2
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 2
            },
            {
                "name": "Standard Comps",
                "compact": "Standard",
                "default": true,
                "area": 3
            },
            {
                "name": "Non-Standard Comps",
                "compact": "Non-Standard",
                "area": 3
            }
        ]
    }
]


/**
 * 获取BOSS数据
 * @param bossNickName BOSS名称
 */

// getEorzeaBoss

const getEorzeaBoss = (bossNickName) => {

    let boss = {
        ok: false,
        bossCnServer: 5,
        difficulty:101
    }

    // 29-伊甸园 28-5.0蛮神 30-巴哈/究极 31-亚历山大

    const zoneId = [23, 29, 28, 30, 31, 32,33]

    if (bossNickName) {

        for (let i = 0; i < zoneId.length; i++) {

            for (let x = 0; x < eorzeaBoss.length; x++) {

                if (eorzeaBoss[x].id == zoneId[i]) {

                    for (let y = 0; y < eorzeaBoss[x].encounters.length; y++) {
                        for (let z = 0; z < eorzeaBoss[x].encounters[y].nickname.length; z++) {

                            if (eorzeaBoss[x].encounters[y].nickname[z] == bossNickName) {

                                boss.ok = true
                                boss.zone = eorzeaBoss[x].id
                                boss.id = eorzeaBoss[x].encounters[y].id
                                boss.name = eorzeaBoss[x].encounters[y].name
                                boss.cnName = eorzeaBoss[x].encounters[y].cnName

                                if (eorzeaBoss[x].encounters[y].bossCnServer) {

                                    boss.bossCnServer = eorzeaBoss[x].encounters[y].bossCnServer

                                }
                                if (eorzeaBoss[x].difficulty) {
                                    boss.difficulty = eorzeaBoss[x].difficulty
                                }
                                // return 0;
                                break;

                            }
                        }
                    }
                }
            }

        }

    }

    return boss

}


module.exports = getEorzeaBoss

// const cboss = getEorzeaBoss("e2s")

// console.log(cboss)